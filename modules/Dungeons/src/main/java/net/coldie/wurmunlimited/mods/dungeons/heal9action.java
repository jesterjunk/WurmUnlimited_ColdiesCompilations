...
public class heal9action implements WurmServerMod, ItemTypes, MiscConstants, ModAction, BehaviourProvider, ActionPerformer {
	...
	public heal9action() {
		...
		actionEntry = ActionEntry.createEntry(actionId, dungeonmain.currencyname9+" Heal", dungeonmain.currencyname9+" Healing", new int[]{ 
				}); 
	...
	public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Creature target) {
		...
		if (dungeonmain.currency9enabled && used == dungeonmain.statuettetemplate9 && target instanceof Player) {
	...
	public boolean action(Action act, Creature performer, Item source, Creature target, short action, float counter) {
        ...
        if (dungeonmain.currency9enabled){
            if (dungeonmain.zone9minx <= X && X <= dungeonmain.zone9maxx && dungeonmain.zone9miny <= Y && Y <= dungeonmain.zone9maxy){
				if (used == dungeonmain.statuettetemplate9 && target instanceof Player) {
					if (target.getBody().getWounds() != null && target.getBody().getWounds().getWounds().length > 0) {
			  	    ...
			  	    if (System.currentTimeMillis()-lastheal > dungeonmain.healcooldown9*1000){
						int power = dungeonmain.healpower9;
						int w9 = 0;
						int numhealed = 0;
						int numfullhealed = 0;
				        Wound[] wounds9 = target.getBody().getWounds().getWounds();
				        if (target.getBody().getWounds() != null && ( wounds9.length > 0)) {
					        	while (w9 < wounds9.length){
					            if (wounds9[w9].getSeverity() / 1000.0f < (float)power) {
					                wounds9[w9].heal();
					                numfullhealed = numfullhealed+1;
					                }else{
					            wounds9[w9].modifySeverity((- power) * 1000);
			  	    ...
			  	    timeleft = dungeonmain.healcooldown9 - timeleft;
            ...
            performer.getCommunicator().sendSafeServerMessage("Nice try, you need to be in "+dungeonmain.currencyname9+" in order to use this item.");
            }
        }
		return true;
	}

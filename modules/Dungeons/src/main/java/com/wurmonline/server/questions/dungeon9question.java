...
import net.coldie.wurmunlimited.mods.dungeons.dungeon9action;
import net.coldie.wurmunlimited.mods.dungeons.dungeonmain;


public class dungeon9question extends Question
{
	  private boolean properlySent = false;
	  
	  dungeon9question(Creature aResponder, String aTitle, String aQuestion, int aType, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, aType, aTarget);
	  }
	  
	  public dungeon9question(Creature aResponder, String aTitle, String aQuestion, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, 79, aTarget);
	  }
	  private static Logger logger = Logger.getLogger(dungeon9question.class.getName());	  
...
public void buyitem(String itemname, int zennycost, int itemid, float ql, byte rarity, int multibuy,byte material){
	...
	int starter = 0;
	String[] dungeon9colorsplit = dungeonmain.dungeon9color.split(",");	        
    int R = Integer.parseInt(dungeon9colorsplit[0]);
    int G = Integer.parseInt(dungeon9colorsplit[1]);
    int B = Integer.parseInt(dungeon9colorsplit[2]);
	...
	starter=starter+1;}
      ...
      {	      
	      ...
	      ps2 = dbcon2.prepareStatement("UPDATE DungeonCurrency SET dungeon9 = dungeon9 - "+zennycost+" WHERE name = \""+charname+"\"");
	      ps2.executeUpdate();
	      ps2.close();
	      dbcon2.close();
	      bought = true;
	      logger.log(Level.INFO, charname+" spent "+zennycost+" "+dungeonmain.currencyname9+" on "+multibuy+"x "+ql+"ql "+itemname);
	      getResponder().getCommunicator().sendNormalServerMessage("You spent "+zennycost+" "+dungeonmain.currencyname9+" on "+multibuy+"x "+ql+"ql "+itemname);
    }
...
//getResponder().getCommunicator().sendNormalServerMessage(val);
			 ...
			 String[] dungeon9itemssplit = dungeonmain.dungeon9items.split(",");
		        int dungeon9itemssplitlength = dungeon9itemssplit.length;
		        int n2 = 0;
		        //name,id,ql,price,rarity
		        while (n2 < dungeon9itemssplitlength) {
		        	int id = Integer.parseInt(dungeon9itemssplit[n2+1]);
		        	float ql = Float.parseFloat(dungeon9itemssplit[n2+2]);
		        	int price = Integer.parseInt(dungeon9itemssplit[n2+3]);
		        	byte rarity = (byte) Integer.parseInt(dungeon9itemssplit[n2+4]);
		        	byte material = (byte) Integer.parseInt(dungeon9itemssplit[n2+5]);
		        	if (val.equals(dungeon9itemssplit[n2]) && zenny >= price*numbertobuy){
		        		buyitem(dungeon9itemssplit[n2],price,id, ql,rarity,numbertobuy,material);
		        	}
		        	n2=n2+6;
			 	}	        
	  ...
	  public void sendQuestion()
	  {
	    boolean ok = true;
	      ...
	      try {
	        ...
	        if (act.getNumber() == dungeon9action.actionId) {
	          ok = true;
	        }
	      }
	    ...
	    if (ok) {
	      ...
	      f.addBoldText("You have "+zenny+" "+dungeonmain.currencyname9+" in your account\n\n",new String[0]);
// add here
	     ...
	     f.addBoldText("\n -- Rarity: 0 = normal, 1 = Rare, 2 = Supreme, 3 = Fantastic --\n", new String[0]);
			 String[] dungeon9itemssplit = dungeonmain.dungeon9items.split(",");
		        ...
		        //name,id,ql,price,rarity
		        while (n2 < dungeon9itemssplitlength) {
		        	int chars = dungeon9itemssplit[n2].length();
		        	String spacer = dungeon9itemssplit[n2];
		        	int counter = 0;
		        	...
		        	//hover='Test this"+dungeon9itemssplit[n2]+"';
					f.addRaw("radio{  group='currency'; id='"+dungeon9itemssplit[n2]+"';	text='"+spacer+"  QL:"+dungeon9itemssplit[n2+2]+"   Price:"+dungeon9itemssplit[n2+3]+"   Rarity:"+dungeon9itemssplit[n2+4]+"'}");
				 	n2=n2+6;
				 	f.endHorizontalFlow();
			 	}

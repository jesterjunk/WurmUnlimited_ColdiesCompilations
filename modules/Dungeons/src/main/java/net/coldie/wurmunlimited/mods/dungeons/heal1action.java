...
public class heal1action implements WurmServerMod, ItemTypes, MiscConstants, ModAction, BehaviourProvider, ActionPerformer {
	...
	public heal1action() {
		...
		actionEntry = ActionEntry.createEntry(actionId, dungeonmain.currencyname1+" Heal", dungeonmain.currencyname1+" Healing", new int[]{ 
				}); 
	...
	public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Creature target) {
		...
		if (dungeonmain.currency1enabled && used == dungeonmain.statuettetemplate1 && target instanceof Player) {
	...
	public boolean action(Action act, Creature performer, Item source, Creature target, short action, float counter) {
        ...
        if (dungeonmain.currency1enabled){
            if (dungeonmain.zone1minx <= X && X <= dungeonmain.zone1maxx && dungeonmain.zone1miny <= Y && Y <= dungeonmain.zone1maxy){
				if (used == dungeonmain.statuettetemplate1 && target instanceof Player) {
					if (target.getBody().getWounds() != null && target.getBody().getWounds().getWounds().length > 0) {
			  	    ...
			  	    if (System.currentTimeMillis()-lastheal > dungeonmain.healcooldown1*1000){
						int power = dungeonmain.healpower1;
						int w2 = 0;
						int numhealed = 0;
						int numfullhealed = 0;
			  	    ...
			  	    timeleft = dungeonmain.healcooldown1 - timeleft;
            ...
            performer.getCommunicator().sendSafeServerMessage("Nice try, you need to be in "+dungeonmain.currencyname1+" in order to use this item.");
            }
        }
		return true;
	}

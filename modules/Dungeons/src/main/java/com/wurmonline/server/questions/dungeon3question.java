...
import net.coldie.wurmunlimited.mods.dungeons.dungeon3action;
import net.coldie.wurmunlimited.mods.dungeons.dungeonmain;


public class dungeon3question extends Question
{
	  private boolean properlySent = false;
	  
	  dungeon3question(Creature aResponder, String aTitle, String aQuestion, int aType, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, aType, aTarget);
	  }
	  
	  public dungeon3question(Creature aResponder, String aTitle, String aQuestion, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, 79, aTarget);
	  }
	  private static Logger logger = Logger.getLogger(dungeon3question.class.getName());	  
...
public void buyitem(String itemname, int zennycost, int itemid, float ql, byte rarity, int multibuy,byte material){
	...
	int starter = 0;
	String[] dungeon3colorsplit = dungeonmain.dungeon3color.split(",");	        
    int R = Integer.parseInt(dungeon3colorsplit[0]);
    int G = Integer.parseInt(dungeon3colorsplit[1]);
    int B = Integer.parseInt(dungeon3colorsplit[2]);
	...
	starter=starter+1;}
      ...
      {	      
	      ...
	      ps2 = dbcon2.prepareStatement("UPDATE DungeonCurrency SET dungeon3 = dungeon3 - "+zennycost+" WHERE name = \""+charname+"\"");
	      ps2.executeUpdate();
	      ps2.close();
	      dbcon2.close();
	      bought = true;
	      logger.log(Level.INFO, charname+" spent "+zennycost+" "+dungeonmain.currencyname3+" on "+multibuy+"x "+ql+"ql "+itemname);
	      getResponder().getCommunicator().sendNormalServerMessage("You spent "+zennycost+" "+dungeonmain.currencyname3+" on "+multibuy+"x "+ql+"ql "+itemname);
    }
...
//getResponder().getCommunicator().sendNormalServerMessage(val);
			 ...
			 String[] dungeon3itemssplit = dungeonmain.dungeon3items.split(",");
		        int dungeon3itemssplitlength = dungeon3itemssplit.length;
		        int n2 = 0;
		        //name,id,ql,price,rarity
		        while (n2 < dungeon3itemssplitlength) {
		        	int id = Integer.parseInt(dungeon3itemssplit[n2+1]);
		        	float ql = Float.parseFloat(dungeon3itemssplit[n2+2]);
		        	int price = Integer.parseInt(dungeon3itemssplit[n2+3]);
		        	byte rarity = (byte) Integer.parseInt(dungeon3itemssplit[n2+4]);
		        	byte material = (byte) Integer.parseInt(dungeon3itemssplit[n2+5]);
		        	if (val.equals(dungeon3itemssplit[n2]) && zenny >= price*numbertobuy){
		        		buyitem(dungeon3itemssplit[n2],price,id, ql,rarity,numbertobuy,material);
		        	}
		        	n2=n2+6;
			 	}	        
	  ...
	  public void sendQuestion()
	  {
	    boolean ok = true;
	      ...
	      try {
	        ...
	        if (act.getNumber() == dungeon3action.actionId) {
	          ok = true;
	        }
	      }
	    ...
	    if (ok) {
	      ...
	      f.addBoldText("You have "+zenny+" "+dungeonmain.currencyname3+" in your account\n\n",new String[0]);
// add here
	     ...
	     f.addBoldText("\n -- Rarity: 0 = normal, 1 = Rare, 2 = Supreme, 3 = Fantastic --\n", new String[0]);
			 String[] dungeon3itemssplit = dungeonmain.dungeon3items.split(",");
		        ...
		        //name,id,ql,price,rarity
		        while (n2 < dungeon3itemssplitlength) {
		        	int chars = dungeon3itemssplit[n2].length();
		        	String spacer = dungeon3itemssplit[n2];
		        	int counter = 0;
		        	...
		        	//hover='Test this"+dungeon3itemssplit[n2]+"';
					f.addRaw("radio{  group='currency'; id='"+dungeon3itemssplit[n2]+"';	text='"+spacer+"  QL:"+dungeon3itemssplit[n2+2]+"   Price:"+dungeon3itemssplit[n2+3]+"   Rarity:"+dungeon3itemssplit[n2+4]+"'}");
				 	n2=n2+6;
				 	f.endHorizontalFlow();
			 	}

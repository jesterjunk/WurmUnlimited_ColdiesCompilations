...
public class heal5action implements WurmServerMod, ItemTypes, MiscConstants, ModAction, BehaviourProvider, ActionPerformer {
	...
	public heal5action() {
		...
		actionEntry = ActionEntry.createEntry(actionId, dungeonmain.currencyname5+" Heal", dungeonmain.currencyname5+" Healing", new int[]{ 
				}); 
	...
	public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Creature target) {
		...
		if (dungeonmain.currency5enabled && used == dungeonmain.statuettetemplate5 && target instanceof Player) {
	...
	public boolean action(Action act, Creature performer, Item source, Creature target, short action, float counter) {
        ...
        if (dungeonmain.currency5enabled){
            if (dungeonmain.zone5minx <= X && X <= dungeonmain.zone5maxx && dungeonmain.zone5miny <= Y && Y <= dungeonmain.zone5maxy){
				if (used == dungeonmain.statuettetemplate5 && target instanceof Player) {
					if (target.getBody().getWounds() != null && target.getBody().getWounds().getWounds().length > 0) {
			  	    ...
			  	    if (System.currentTimeMillis()-lastheal > dungeonmain.healcooldown5*1000){
						int power = dungeonmain.healpower5;
						int w5 = 0;
						int numhealed = 0;
						int numfullhealed = 0;
				        Wound[] wounds5 = target.getBody().getWounds().getWounds();
				        if (target.getBody().getWounds() != null && ( wounds5.length > 0)) {
					        	while (w5 < wounds5.length){
					            if (wounds5[w5].getSeverity() / 1000.0f < (float)power) {
					                wounds5[w5].heal();
					                numfullhealed = numfullhealed+1;
					                }else{
					            wounds5[w5].modifySeverity((- power) * 1000);
			  	    ...
			  	    timeleft = dungeonmain.healcooldown5 - timeleft;
            ...
            performer.getCommunicator().sendSafeServerMessage("Nice try, you need to be in "+dungeonmain.currencyname5+" in order to use this item.");
            }
        }
		return true;
	}

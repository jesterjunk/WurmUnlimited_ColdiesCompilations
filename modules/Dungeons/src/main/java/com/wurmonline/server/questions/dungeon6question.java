...
import net.coldie.wurmunlimited.mods.dungeons.dungeon6action;
import net.coldie.wurmunlimited.mods.dungeons.dungeonmain;


public class dungeon6question extends Question
{
	  private boolean properlySent = false;
	  
	  dungeon6question(Creature aResponder, String aTitle, String aQuestion, int aType, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, aType, aTarget);
	  }
	  
	  public dungeon6question(Creature aResponder, String aTitle, String aQuestion, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, 79, aTarget);
	  }
	  private static Logger logger = Logger.getLogger(dungeon6question.class.getName());	  
...
public void buyitem(String itemname, int zennycost, int itemid, float ql, byte rarity, int multibuy, byte material){
	...
	int starter = 0;
	String[] dungeon6colorsplit = dungeonmain.dungeon6color.split(",");	        
    int R = Integer.parseInt(dungeon6colorsplit[0]);
    int G = Integer.parseInt(dungeon6colorsplit[1]);
    int B = Integer.parseInt(dungeon6colorsplit[2]);
	...
	starter=starter+1;}
      ...
      {	      
	      ...
	      ps2 = dbcon2.prepareStatement("UPDATE DungeonCurrency SET dungeon6 = dungeon6 - "+zennycost+" WHERE name = \""+charname+"\"");
	      ps2.executeUpdate();
	      ps2.close();
	      dbcon2.close();
	      bought = true;
	      logger.log(Level.INFO, charname+" spent "+zennycost+" "+dungeonmain.currencyname6+" on "+multibuy+"x "+ql+"ql "+itemname);
	      getResponder().getCommunicator().sendNormalServerMessage("You spent "+zennycost+" "+dungeonmain.currencyname6+" on "+multibuy+"x "+ql+"ql "+itemname);
    }
...
//getResponder().getCommunicator().sendNormalServerMessage(val);
			 ...
			 String[] dungeon6itemssplit = dungeonmain.dungeon6items.split(",");
		        int dungeon6itemssplitlength = dungeon6itemssplit.length;
		        int n2 = 0;
		        //name,id,ql,price,rarity
		        while (n2 < dungeon6itemssplitlength) {
		        	int id = Integer.parseInt(dungeon6itemssplit[n2+1]);
		        	float ql = Float.parseFloat(dungeon6itemssplit[n2+2]);
		        	int price = Integer.parseInt(dungeon6itemssplit[n2+3]);
		        	byte rarity = (byte) Integer.parseInt(dungeon6itemssplit[n2+4]);
		        	byte material = (byte) Integer.parseInt(dungeon6itemssplit[n2+5]);
		        	if (val.equals(dungeon6itemssplit[n2]) && zenny >= price*numbertobuy){
		        		buyitem(dungeon6itemssplit[n2],price,id, ql,rarity,numbertobuy,material);
		        	}
		        	n2=n2+6;
			 	}	        
	  ...
	  public void sendQuestion()
	  {
	    boolean ok = true;
	      ...
	      try {
	        ...
	        if (act.getNumber() == dungeon6action.actionId) {
	          ok = true;
	        }
	      }
	    ...
	    if (ok) {
	      ...
	      f.addBoldText("You have "+zenny+" "+dungeonmain.currencyname6+" in your account\n\n",new String[0]);
// add here
	     ...
	     f.addBoldText("\n -- Rarity: 0 = normal, 1 = Rare, 2 = Supreme, 3 = Fantastic --\n", new String[0]);
			 String[] dungeon6itemssplit = dungeonmain.dungeon6items.split(",");
		        ...
		        //name,id,ql,price,rarity
		        while (n2 < dungeon6itemssplitlength) {
		        	int chars = dungeon6itemssplit[n2].length();
		        	String spacer = dungeon6itemssplit[n2];
		        	int counter = 0;
		        	...
		        	//hover='Test this"+dungeon6itemssplit[n2]+"';
					f.addRaw("radio{  group='currency'; id='"+dungeon6itemssplit[n2]+"';	text='"+spacer+"  QL:"+dungeon6itemssplit[n2+2]+"   Price:"+dungeon6itemssplit[n2+3]+"   Rarity:"+dungeon6itemssplit[n2+4]+"'}");
				 	n2=n2+6;
				 	f.endHorizontalFlow();
			 	}

...
public class eventreloadaction implements ModAction, BehaviourProvider, ActionPerformer {
	public final short actionId;
	public final ActionEntry actionEntry;

	public eventreloadaction() {
		actionId = (short) ModActions.getNextActionId();
		actionEntry = ActionEntry.createEntry(actionId, "Reload event properties", "Reloading event properties", new int[0]);
	...
	public boolean action(Action act, Creature performer, Item target, short action, float counter) {
		if (performer instanceof Player && performer.getPower() > 4) {
			Path path = Paths.get("mods/eventmod.properties");

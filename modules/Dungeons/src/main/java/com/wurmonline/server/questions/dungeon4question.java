...
import net.coldie.wurmunlimited.mods.dungeons.dungeon4action;
import net.coldie.wurmunlimited.mods.dungeons.dungeonmain;


public class dungeon4question extends Question
{
	  private boolean properlySent = false;
	  
	  dungeon4question(Creature aResponder, String aTitle, String aQuestion, int aType, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, aType, aTarget);
	  }
	  
	  public dungeon4question(Creature aResponder, String aTitle, String aQuestion, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, 79, aTarget);
	  }
	  private static Logger logger = Logger.getLogger(dungeon4question.class.getName());	  
...
public void buyitem(String itemname, int zennycost, int itemid, float ql, byte rarity, int multibuy,byte material){
	...
	int starter = 0;
	String[] dungeon4colorsplit = dungeonmain.dungeon4color.split(",");	        
    int R = Integer.parseInt(dungeon4colorsplit[0]);
    int G = Integer.parseInt(dungeon4colorsplit[1]);
    int B = Integer.parseInt(dungeon4colorsplit[2]);
	...
	starter=starter+1;}
      ...
      {	      
	      ...
	      ps2 = dbcon2.prepareStatement("UPDATE DungeonCurrency SET dungeon4 = dungeon4 - "+zennycost+" WHERE name = \""+charname+"\"");
	      ps2.executeUpdate();
	      ps2.close();
	      dbcon2.close();
	      bought = true;
	      logger.log(Level.INFO, charname+" spent "+zennycost+" "+dungeonmain.currencyname4+" on "+multibuy+"x "+ql+"ql "+itemname);
	      getResponder().getCommunicator().sendNormalServerMessage("You spent "+zennycost+" "+dungeonmain.currencyname4+" on "+multibuy+"x "+ql+"ql "+itemname);
    }
...
//getResponder().getCommunicator().sendNormalServerMessage(val);
			 ...
			 String[] dungeon4itemssplit = dungeonmain.dungeon4items.split(",");
		        int dungeon4itemssplitlength = dungeon4itemssplit.length;
		        int n2 = 0;
		        //name,id,ql,price,rarity
		        while (n2 < dungeon4itemssplitlength) {
		        	int id = Integer.parseInt(dungeon4itemssplit[n2+1]);
		        	float ql = Float.parseFloat(dungeon4itemssplit[n2+2]);
		        	int price = Integer.parseInt(dungeon4itemssplit[n2+3]);
		        	byte rarity = (byte) Integer.parseInt(dungeon4itemssplit[n2+4]);
		        	byte material = (byte) Integer.parseInt(dungeon4itemssplit[n2+5]);
		        	if (val.equals(dungeon4itemssplit[n2]) && zenny >= price*numbertobuy){
		        		buyitem(dungeon4itemssplit[n2],price,id, ql,rarity,numbertobuy,material);
		        	}
		        	n2=n2+6;
			 	}	        
	  ...
	  public void sendQuestion()
	  {
	    boolean ok = true;
	      ...
	      try {
	        ...
	        if (act.getNumber() == dungeon4action.actionId) {
	          ok = true;
	        }
	      }
	    ...
	    if (ok) {
	      ...
	      f.addBoldText("You have "+zenny+" "+dungeonmain.currencyname4+" in your account\n\n",new String[0]);
// add here
	     ...
	     f.addBoldText("\n -- Rarity: 0 = normal, 1 = Rare, 2 = Supreme, 3 = Fantastic --\n", new String[0]);
			 String[] dungeon4itemssplit = dungeonmain.dungeon4items.split(",");
		        ...
		        //name,id,ql,price,rarity
		        while (n2 < dungeon4itemssplitlength) {
		        	int chars = dungeon4itemssplit[n2].length();
		        	String spacer = dungeon4itemssplit[n2];
		        	int counter = 0;
		        	...
		        	//hover='Test this"+dungeon4itemssplit[n2]+"';
					f.addRaw("radio{  group='currency'; id='"+dungeon4itemssplit[n2]+"';	text='"+spacer+"  QL:"+dungeon4itemssplit[n2+2]+"   Price:"+dungeon4itemssplit[n2+3]+"   Rarity:"+dungeon4itemssplit[n2+4]+"'}");
				 	n2=n2+6;
				 	f.endHorizontalFlow();
			 	}

...
public class playerreturnquestion extends Question {
	private boolean properlySent = false;

	playerreturnquestion(Creature aResponder, String aTitle, String aQuestion, int aType, long aTarget) {
		super(aResponder, aTitle, aQuestion, aType, aTarget);
	}

	public playerreturnquestion(Creature aResponder, String aTitle, String aQuestion, long aTarget) {
	...
	public void answer(Properties answer) {
		...
		if (accepted) {
			...
			try {
				...
				ps = dbcon.prepareStatement("SELECT * FROM ColdieEventPortals WHERE id=?");
			...
			try {
				dbcon = ModSupportDb.getModSupportDb();
				ps = dbcon.prepareStatement("UPDATE ColdieEventPortals SET posx=1 WHERE id=?");
				ps.setLong(1, getResponder().getWurmId());
				ps.executeUpdate();
			}
			catch (SQLException e) { throw new RuntimeException(e); }
			...
			//getResponder().getCommunicator().sendNormalServerMessage("Teleporting to: "+tx+", "+ty);
			getResponder().setTeleportPoints(tx, ty, layer, floorlevel);
			getResponder().getCommunicator().sendNormalServerMessage("floor: "+floorlevel);
	...
	public void sendQuestion() {
		...
		try {
			...
			if (act.getNumber() == eventmod.getReturnEventActionId())
				ok = true;
		}

...
public class LTreload implements ModAction, BehaviourProvider, ActionPerformer {
	public final short actionId;
	public final ActionEntry actionEntry;

	public LTreload() {
		actionId = (short) ModActions.getNextActionId();
		actionEntry = ActionEntry.createEntry(actionId, "Reload LT properties", "Reloading LT properties", new int[0]);
	...
	public boolean action(Action act, Creature performer, Item target, short action, float counter) {
		if (performer instanceof Player && performer.getPower() > 4) {
			Path path = Paths.get("mods/LTdebuff.properties");
			...
			try {
				...
				LTdebuff.doconfig(properties);
				LTdebuff.logger.info("Reloading configuration.");
				performer.getCommunicator().sendAlertServerMessage("Loading all options.");
			} catch (IOException ex) {
				LTdebuff.logger.log(Level.SEVERE, "Error while reloading properties file.", ex);
				...
				try {
					...
					LTdebuff.logger.log(Level.SEVERE, "Properties file not closed, possible file lock.", ex);

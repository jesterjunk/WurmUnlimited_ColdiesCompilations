...
import net.coldie.wurmunlimited.mods.dungeons.dungeon8action;
import net.coldie.wurmunlimited.mods.dungeons.dungeonmain;


public class dungeon8question extends Question
{
	  private boolean properlySent = false;
	  
	  dungeon8question(Creature aResponder, String aTitle, String aQuestion, int aType, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, aType, aTarget);
	  }
	  
	  public dungeon8question(Creature aResponder, String aTitle, String aQuestion, long aTarget)
	  {
	    super(aResponder, aTitle, aQuestion, 79, aTarget);
	  }
	  private static Logger logger = Logger.getLogger(dungeon8question.class.getName());	  
...
public void buyitem(String itemname, int zennycost, int itemid, float ql, byte rarity, int multibuy,byte material){
	...
	int starter = 0;
	String[] dungeon8colorsplit = dungeonmain.dungeon8color.split(",");	        
    int R = Integer.parseInt(dungeon8colorsplit[0]);
    int G = Integer.parseInt(dungeon8colorsplit[1]);
    int B = Integer.parseInt(dungeon8colorsplit[2]);
	...
	starter=starter+1;}
      ...
      {	      
	      ...
	      ps2 = dbcon2.prepareStatement("UPDATE DungeonCurrency SET dungeon8 = dungeon8 - "+zennycost+" WHERE name = \""+charname+"\"");
	      ps2.executeUpdate();
	      ps2.close();
	      dbcon2.close();
	      bought = true;
	      logger.log(Level.INFO, charname+" spent "+zennycost+" "+dungeonmain.currencyname8+" on "+multibuy+"x "+ql+"ql "+itemname);
	      getResponder().getCommunicator().sendNormalServerMessage("You spent "+zennycost+" "+dungeonmain.currencyname8+" on "+multibuy+"x "+ql+"ql "+itemname);
    }
...
//getResponder().getCommunicator().sendNormalServerMessage(val);
			 ...
			 String[] dungeon8itemssplit = dungeonmain.dungeon8items.split(",");
		        int dungeon8itemssplitlength = dungeon8itemssplit.length;
		        int n2 = 0;
		        //name,id,ql,price,rarity
		        while (n2 < dungeon8itemssplitlength) {
		        	int id = Integer.parseInt(dungeon8itemssplit[n2+1]);
		        	float ql = Float.parseFloat(dungeon8itemssplit[n2+2]);
		        	int price = Integer.parseInt(dungeon8itemssplit[n2+3]);
		        	byte rarity = (byte) Integer.parseInt(dungeon8itemssplit[n2+4]);
		        	byte material = (byte) Integer.parseInt(dungeon8itemssplit[n2+5]);
		        	if (val.equals(dungeon8itemssplit[n2]) && zenny >= price*numbertobuy){
		        		buyitem(dungeon8itemssplit[n2],price,id, ql,rarity,numbertobuy,material);
		        	}
		        	n2=n2+6;
			 	}	        
	  ...
	  public void sendQuestion()
	  {
	    boolean ok = true;
	      ...
	      try {
	        ...
	        if (act.getNumber() == dungeon8action.actionId) {
	          ok = true;
	        }
	      }
	    ...
	    if (ok) {
	      ...
	      f.addBoldText("You have "+zenny+" "+dungeonmain.currencyname8+" in your account\n\n",new String[0]);
// add here
	     ...
	     f.addBoldText("\n -- Rarity: 0 = normal, 1 = Rare, 2 = Supreme, 3 = Fantastic --\n", new String[0]);
			 String[] dungeon8itemssplit = dungeonmain.dungeon8items.split(",");
		        ...
		        //name,id,ql,price,rarity
		        while (n2 < dungeon8itemssplitlength) {
		        	int chars = dungeon8itemssplit[n2].length();
		        	String spacer = dungeon8itemssplit[n2];
		        	int counter = 0;
		        	...
		        	//hover='Test this"+dungeon8itemssplit[n2]+"';
					f.addRaw("radio{  group='currency'; id='"+dungeon8itemssplit[n2]+"';	text='"+spacer+"  QL:"+dungeon8itemssplit[n2+2]+"   Price:"+dungeon8itemssplit[n2+3]+"   Rarity:"+dungeon8itemssplit[n2+4]+"'}");
				 	n2=n2+6;
				 	f.endHorizontalFlow();
			 	}
